(function () {
  ...
  /**
   * @summary Check if the style element exists and that it has MS Dev Theme color content
  ...
  /**
   * @summary Checks if a theme is applied, and that the theme belongs to the MS Dev family
   * @returns {boolean}
   */
  const usingMSDevTheme = () => {
    const appliedTheme = document.querySelector('[class*="theme-json"]');
    const MSDevTheme = document.querySelector('[class*="MohamedSuliman-msdevtheme-vscode-themes"]');
    return appliedTheme && MSDevTheme;
  }

  /**
   * @summary Checks if the theme is msdevtheme, and that the styles exist, ready for replacement
   * @param {HTMLElement} tokensEl the style tag
   * @param {object} replacements key/value pairs of colour hex and the glow styles to replace them with
   * @returns 
   */
  const readyForReplacement = (tokensEl, tokenReplacements) => tokensEl 
    ? (
      // only init if we're using a MS Dev Theme subtheme
      usingMSDevTheme() &&         
  ...
  const initNeonDreams = (disableGlow, obs) => {
    ...
    if (!document.querySelector('#msdevtheme-styles')) {
      ...
      newStyleTag.setAttribute("id", "msdevtheme-styles");
      newStyleTag.innerText = updatedThemeStyles.replace(/(\r\n|\n|\r)/gm, '');
      document.body.appendChild(newStyleTag);
      
      console.log('MS Dev Theme: NEON DREAMS initialised!');
    }
