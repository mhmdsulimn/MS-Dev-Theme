 ...
 * @param {vscode.ViewColumn} viewColumn العمود الذي ستظهر فيه المعاينة (اختياري).
 */
...
/**
 * يعرض رسالة معلوماتية للمستخدم مع خيار فتح ملف Markdown.
 * @param {vscode.ExtensionContext} context
 * @param {string} message
 * @param {string} mdFileName
 */
async function infoWithOptionalMarkdown(context, message, mdFileName) {
    const selection = await vscode.window.showInformationMessage(
        message,
        'نعم', 'لاحقًا'
    );
    if (selection === 'نعم') {
        await showMarkdownPreview(context, mdFileName, vscode.ViewColumn.One);
    }
}
...
function activate(context) {
    ...
    setTimeout(async () => {
        if (!installedBefore) {
            // أول تثبيت
            console.log('MS Dev Theme: First install detected.');
            await infoWithOptionalMarkdown(
                context,
                '🎉 شكرًا لتثبيت الاضافه الخاصه بنا هل تريد مشاهدة مقدمة سريعة؟',
                'THANK_YOU.md'
            );
            ...
            // تم التحديث
            console.log(`MS Dev Theme: Update detected from ${lastVersionShown} to ${currentVersion}.`);
            await infoWithOptionalMarkdown(
                context,
                `✨ تم تحديث الاضافه إلى الإصدار ${currentVersion}. هل تريد معرفة الجديد؟`,
                'WHATS_NEW.md'
            );
            await context.globalState.update(LAST_VERSION_KEY, currentVersion);
        }
    ...
    const showWhatsNewDisposable = vscode.commands.registerCommand('msdevtheme.showWhatsNew', async () => {
        await showMarkdownPreview(context, 'WHATS_NEW.md');
    });
    context.subscriptions.push(showWhatsNewDisposable);

    const showThankYouDisposable = vscode.commands.registerCommand('msdevtheme.showThankYouPage', async () => {
    ...
    vscode.commands.registerCommand('msdevtheme.resetState', async () => {
        await context.globalState.update(INSTALLED_BEFORE_KEY, undefined);
        await context.globalState.update(LAST_VERSION_KEY, undefined);
        vscode.window.showInformationMessage('تمت إعادة تعيين حالة الإضافة.');
    });
}

function deactivate() {
    // إمكانية وضع كود تنظيف عند الحاجة
}
